...
class Enemy {
  ...
  void update() {
    ...
    if (fadeStart > 0) { 
      fade();
    }  //Fade when fade amount is set.
    if (isDead) {
      if (checkBlockCollision() || destroy) { 
        deadEnemies.remove(this);
      }  //Remove enemy death projectile on impact.
    }
  }
  ...
  void damageEnemy(Shot _shot) {
    ...
    if (lifes <= 0 && !isDead) {
      ...
      if (random(0, 100) > 75) {
        ...
        doAnimation = false;
      }  //Projectiles are still animated.
