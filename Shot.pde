...
class Shot {
  ...
  void update() {
    ...
    else { lastMove += millis() - lastMove; }  //Adjust lastMove in order to pause shot.
    ...
    if(shotDir.y < 0) {
      ...
      for(int i = players.size() - 1; i > -1; i--) {
        ...
        if(!_player.isDead && collisionCheck(shotPos, 0, 0, new PVector(_player.x, _player.y), _player.pWidth/2, _player.pHeight)) {
          if(_player.hasShield) { _player.hasShield = false; }  //If player has shield then remove it,
          else { _player.adjustLifes(); }  //otherwise damage player and destroy shot.
          return true;
        }
      }
    }
